...
class resnetv1(Network):
  ...
  def _crop_pool_layer(self, bottom, rois, name):
    with tf.variable_scope(name) as scope:
      ...
      diffx = (x2-x1)/4
      diffy = (y2-y1)/4
      bboxes1 = tf.stop_gradient(tf.concat([y1+diffy, x1+diffx, y2-diffy, x2-diffx], 1))
      if cfg.RESNET.MAX_POOL:
        ...
        crops1 = tf.image.crop_and_resize(bottom, bboxes, tf.to_int32(batch_ids), [cfg.POOLING_SIZE, cfg.POOLING_SIZE],
                                         name="crops1")
        crops2 = tf.image.crop_and_resize(bottom, bboxes1, tf.to_int32(batch_ids), [cfg.POOLING_SIZE, cfg.POOLING_SIZE],
                                         name="crops2")
  ...
  def _build_base_reid(self):
    with tf.variable_scope(self._scope, self._scope):

      net = resnet_utils.conv2d_same(self._image_reid, 64, 7, stride=2, scope='conv1')
      net = tf.pad(net, [[0, 0], [1, 1], [1, 1], [0, 0]])
      net = slim.max_pool2d(net, [3, 3], stride=2, padding='VALID', scope='pool1')

    return net

  def _image_to_head_reid(self, is_training, reuse=False):
    assert (0 <= cfg.RESNET.FIXED_BLOCKS <= 3)
    # Now the base is always fixed during training
    with slim.arg_scope(resnet_arg_scope(is_training=False)):
      net_conv = self._build_base_reid()
    if cfg.RESNET.FIXED_BLOCKS > 0:
      with slim.arg_scope(resnet_arg_scope(is_training=False)):
        net_conv, _ = resnet_v1.resnet_v1(net_conv,
                                           self._blocks[0:cfg.RESNET.FIXED_BLOCKS],
