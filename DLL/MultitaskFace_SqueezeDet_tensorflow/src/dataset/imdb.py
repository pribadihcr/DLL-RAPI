...
class imdb(object):
  ...
  def read_batch(self, shuffle=True):
    ...
    for idx in batch_idx:
      ...
      pose_per_batch.append([b[0] for b in self._poseclass[idx][:]])
      age_per_batch.append([b[0] for b in self._ageclass[idx][:]])
      gt_bbox = np.array([[b[0], b[1], b[2], b[3]] for b in self._rois[idx][:]])
      gt_landmark = np.array([[k[0], k[1], k[2], k[3], k[4], k[5],
                             k[6], k[7], k[8], k[9]] for k in self._keypoints[idx][:]])
      ...
      gt_landmark[:,0:5] = gt_landmark[:,0:5]*x_scale
      gt_landmark[:,5:10] = gt_landmark[:,5:10]*y_scale
      landmark_per_batch.append(gt_landmark)

      aidx_per_image, delta_per_image, landmark_delta_per_image= [], [], []
      aidx_set = set()
      for i in range(len(gt_bbox)):
        ...
        p_x0, p_x1, p_x2, p_x3, p_x4, p_y0, p_y1, p_y2, p_y3, p_y4 = gt_landmark[i]
        landmark_delta = [0]*mc.POINTS
        landmark_delta[0] = (p_x0 -(mc.ANCHOR_BOX[aidx][0])) / mc.ANCHOR_BOX[aidx][2] #+(box_cx - p_x0)
        landmark_delta[1] = (p_x1 - (mc.ANCHOR_BOX[aidx][0])) / mc.ANCHOR_BOX[aidx][2]
        landmark_delta[2] = (p_x2 - (mc.ANCHOR_BOX[aidx][0])) / mc.ANCHOR_BOX[aidx][2]
        landmark_delta[3] = (p_x3 - (mc.ANCHOR_BOX[aidx][0])) / mc.ANCHOR_BOX[aidx][2]
        landmark_delta[4] = (p_x4 - (mc.ANCHOR_BOX[aidx][0])) / mc.ANCHOR_BOX[aidx][2]

        landmark_delta[5] = (p_y0 - (mc.ANCHOR_BOX[aidx][1])) / mc.ANCHOR_BOX[aidx][3]
        landmark_delta[6] = (p_y1 - (mc.ANCHOR_BOX[aidx][1])) / mc.ANCHOR_BOX[aidx][3]
        landmark_delta[7] = (p_y2 - (mc.ANCHOR_BOX[aidx][1])) / mc.ANCHOR_BOX[aidx][3]
        landmark_delta[8] = (p_y3 - (mc.ANCHOR_BOX[aidx][1]))/ mc.ANCHOR_BOX[aidx][3]
        landmark_delta[9] = (p_y4 - (mc.ANCHOR_BOX[aidx][1])) / mc.ANCHOR_BOX[aidx][3]

        landmark_delta_per_image.append(landmark_delta)

      delta_per_batch.append(delta_per_image)
      landmark_delta_per_batch.append(landmark_delta_per_image)
    ...
    return image_per_batch, label_per_batch, pose_per_batch, age_per_batch, delta_per_batch, \
        aidx_per_batch, bbox_per_batch, landmark_delta_per_batch, landmark_per_batch
