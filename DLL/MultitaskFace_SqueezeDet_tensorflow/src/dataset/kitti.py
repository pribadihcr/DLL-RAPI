...
class kitti(imdb):
  def __init__(self, image_set, data_path, mc):
    ...
    self._poses = self.mc.POSE_NAMES
    self._ages = self.mc.AGE_NAMES
    self._class_to_idx = dict(zip(self.classes, xrange(self.num_classes)))
    self._pose_to_idx = dict(zip(self._poses, xrange(2)))
    self._age_to_idx = dict(zip(self._ages, xrange(2)))
    ...
    self._rois, self._keypoints, self._poseclass, self._ageclass = self._load_annotation()
    end_time = time.time() - start_time
    print("elapsed time read data:", end_time)
