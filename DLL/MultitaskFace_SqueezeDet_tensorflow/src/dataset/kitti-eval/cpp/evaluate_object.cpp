...
enum CLASSES{CRY=0, CUTE=1, NORMAL=2, SLEEP=3, WAKEUP=4, FACEADULT=5};
...
void initGlobals () {
  ...
  CLASS_NAMES.push_back("01cry");
  CLASS_NAMES.push_back("02cute");
  CLASS_NAMES.push_back("03normal");
  CLASS_NAMES.push_back("04sleep");
  CLASS_NAMES.push_back("05wakeup");
  CLASS_NAMES.push_back("06faceadult");
}
...
vector<tDetection> loadDetections(string file_name, bool &compute_aos, bool &eval_cry, bool &eval_cute, bool &eval_normal, bool &eval_sleep, bool &eval_wakeup, bool &eval_faceadult, bool &success) {
  ...
  while (!feof(fp)) {
    ...
    if (fscanf(fp, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
      ...
      //  eval_cyclist = true;
      if(!eval_cry && !strcasecmp(d.box.type.c_str(), "01cry"))
        eval_cry = true;
      if(!eval_cute && !strcasecmp(d.box.type.c_str(), "02cute"))
        eval_cute = true;
      if(!eval_normal && !strcasecmp(d.box.type.c_str(), "03normal"))
        eval_normal = true;
      if(!eval_sleep && !strcasecmp(d.box.type.c_str(), "04sleep"))
        eval_sleep = true;
      if(!eval_wakeup && !strcasecmp(d.box.type.c_str(), "05wakeup"))
        eval_wakeup = true;
      if(!eval_faceadult && !strcasecmp(d.box.type.c_str(), "06faceadult"))
        eval_faceadult = true;
    }
  }
  fclose(fp);
...
//  string result_dir     = "results/" + result_sha;
  ...
  bool compute_aos=true, eval_cry=false, eval_cute=false, eval_normal=false, eval_sleep=false, eval_wakeup=false;
  ...
  for (int32_t i=0; i<N_TESTIMAGES; i++) {
    ...
    vector<tDetection>   det  = loadDetections(ospj(result_dir,"data",file_name), compute_aos, eval_cry, eval_cute, eval_normal, eval_sleep,eval_wakeup, eval_faceadult, det_success);
  ...
  // eval cry
  if(eval_cry){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[CRY] + "_detection.txt").c_str(),"w");
    fp_ap = fopen((result_dir + "/stats_" + CLASS_NAMES[CRY] + "_ap.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[CRY] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if( !eval_class(fp_det,fp_ap,fp_ori,CRY,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,CRY,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,CRY,groundtruth,detections,compute_aos,precision[2],aos[2],HARD, N_TESTIMAGES)){
      mail->msg("Cry evaluation failed.");
      return false;
    }
    fclose(fp_det);
    fclose(fp_ap);
    saveAndPlotPlots(plot_dir,CLASS_NAMES[CRY] + "_detection",CLASS_NAMES[CRY],precision,0);
    if(compute_aos){
      saveAndPlotPlots(plot_dir,CLASS_NAMES[CRY] + "_orientation",CLASS_NAMES[CRY],aos,1);
      fclose(fp_ori);
    }
  }
    ...
    // eval cute
  if(eval_cute){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[CUTE] + "_detection.txt").c_str(),"w");
    fp_ap = fopen((result_dir + "/stats_" + CLASS_NAMES[CUTE] + "_ap.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[CUTE] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if( !eval_class(fp_det,fp_ap,fp_ori,CUTE,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,CUTE,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,CUTE,groundtruth,detections,compute_aos,precision[2],aos[2],HARD, N_TESTIMAGES)){
      mail->msg("cute evaluation failed.");
      return false;
    }
    fclose(fp_det);
    fclose(fp_ap);
    saveAndPlotPlots(plot_dir,CLASS_NAMES[CUTE] + "_detection",CLASS_NAMES[CUTE],precision,0);
    if(compute_aos){
      saveAndPlotPlots(plot_dir,CLASS_NAMES[CUTE] + "_orientation",CLASS_NAMES[CUTE],aos,1);
      fclose(fp_ori);
    }
  }
    ...
    // eval NORMAL
  if(eval_normal){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[NORMAL] + "_detection.txt").c_str(),"w");
    fp_ap = fopen((result_dir + "/stats_" + CLASS_NAMES[NORMAL] + "_ap.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[NORMAL] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if( !eval_class(fp_det,fp_ap,fp_ori,NORMAL,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,NORMAL,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,NORMAL,groundtruth,detections,compute_aos,precision[2],aos[2],HARD, N_TESTIMAGES)){
      mail->msg("normal evaluation failed.");
      return false;
    }
    fclose(fp_det);
    fclose(fp_ap);
    saveAndPlotPlots(plot_dir,CLASS_NAMES[NORMAL] + "_detection",CLASS_NAMES[NORMAL],precision,0);
    if(compute_aos){
      saveAndPlotPlots(plot_dir,CLASS_NAMES[NORMAL] + "_orientation",CLASS_NAMES[NORMAL],aos,1);
      fclose(fp_ori);
    }
  }
    ...
    // eval SLEEP
  if(eval_sleep){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[SLEEP] + "_detection.txt").c_str(),"w");
    fp_ap = fopen((result_dir + "/stats_" + CLASS_NAMES[SLEEP] + "_ap.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[SLEEP] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if( !eval_class(fp_det,fp_ap,fp_ori,SLEEP,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,SLEEP,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,SLEEP,groundtruth,detections,compute_aos,precision[2],aos[2],HARD, N_TESTIMAGES)){
      mail->msg("sleep evaluation failed.");
      return false;
    }
    fclose(fp_det);
    fclose(fp_ap);
    saveAndPlotPlots(plot_dir,CLASS_NAMES[SLEEP] + "_detection",CLASS_NAMES[SLEEP],precision,0);
    if(compute_aos){
      saveAndPlotPlots(plot_dir,CLASS_NAMES[SLEEP] + "_orientation",CLASS_NAMES[SLEEP],aos,1);
      fclose(fp_ori);
    }
  }
    ...
    // eval WAKEUP
  if(eval_wakeup){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[WAKEUP] + "_detection.txt").c_str(),"w");
    fp_ap = fopen((result_dir + "/stats_" + CLASS_NAMES[WAKEUP] + "_ap.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[WAKEUP] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if( !eval_class(fp_det,fp_ap,fp_ori,WAKEUP,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,WAKEUP,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,WAKEUP,groundtruth,detections,compute_aos,precision[2],aos[2],HARD, N_TESTIMAGES)){
      mail->msg("wakeup evaluation failed.");
      return false;
    }
    fclose(fp_det);
    fclose(fp_ap);
    saveAndPlotPlots(plot_dir,CLASS_NAMES[WAKEUP] + "_detection",CLASS_NAMES[WAKEUP],precision,0);
    if(compute_aos){
      saveAndPlotPlots(plot_dir,CLASS_NAMES[WAKEUP] + "_orientation",CLASS_NAMES[WAKEUP],aos,1);
      fclose(fp_ori);
    }
  }
    ...
    // eval FACEADULT
  if(eval_faceadult){
    fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[FACEADULT] + "_detection.txt").c_str(),"w");
    fp_ap = fopen((result_dir + "/stats_" + CLASS_NAMES[FACEADULT] + "_ap.txt").c_str(),"w");
    if(compute_aos)
      fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[FACEADULT] + "_orientation.txt").c_str(),"w");
    vector<double> precision[3], aos[3];
    if( !eval_class(fp_det,fp_ap,fp_ori,FACEADULT,groundtruth,detections,compute_aos,precision[0],aos[0],EASY,N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,FACEADULT,groundtruth,detections,compute_aos,precision[1],aos[1],MODERATE, N_TESTIMAGES)
       || !eval_class(fp_det,fp_ap,fp_ori,FACEADULT,groundtruth,detections,compute_aos,precision[2],aos[2],HARD, N_TESTIMAGES)){
      mail->msg("faceadult evaluation failed.");
      return false;
    }
    fclose(fp_det);
    fclose(fp_ap);
    saveAndPlotPlots(plot_dir,CLASS_NAMES[FACEADULT] + "_detection",CLASS_NAMES[FACEADULT],precision,0);
    if(compute_aos){
      saveAndPlotPlots(plot_dir,CLASS_NAMES[FACEADULT] + "_orientation",CLASS_NAMES[FACEADULT],aos,1);
      fclose(fp_ori);
    }
  }
  // success
